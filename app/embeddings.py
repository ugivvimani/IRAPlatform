...
class OpenAIEmbeddings(EmbeddingModel):
    ...
    def embed_sync(self, texts: list[str]) -> list[list[float]]:
        ...
        if not self.client:
            logger.warning("OpenAI client not initialized; using local fallback")
            local_model = LocalEmbeddings()
            return local_model.embed_sync(texts)
...
class HybridEmbeddings(EmbeddingModel):
    ...
    def __init__(
        ...
        self.local = LocalEmbeddings(model_name=local_model)
    ...
    def embed_sync(self, texts: list[str]) -> list[list[float]]:
        ...
        return self.local.embed_sync(texts)
...
class EmbeddingFactory:
    ...
    @staticmethod
    def create(embedding_type: str = "hybrid") -> EmbeddingModel:
        """
        Create embedding model based on type.
        
        Args:
            embedding_type: "openai", "local", or "hybrid" (default)
        
        Returns:
            Configured embedding model
        """
        embedding_type = embedding_type or os.getenv("EMBEDDING_TYPE", "hybrid")
